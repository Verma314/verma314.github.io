...
The columns tell us where the standard basis vectors land:
...
The coefficients (3 and 2) stayed the same. The matrix just told us the new destinations for each basis vector.


Each column of the matrix is where one basis vector lands after the transformation. The coefficients stay the same: the matrix just tells you the new destinations.
